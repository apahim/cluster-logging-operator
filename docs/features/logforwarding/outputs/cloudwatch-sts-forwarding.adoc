...
spec:
  ...
  outputs:
    ...
    - name: cw-with-assume-role
      type: cloudwatch
      cloudwatch:
        groupName: 'cw-cross-account{.log_type||"missing"}'
        region: us-west-1
        authentication:
          type: iamRole
          iamRole:
            ...
            token:
              from: serviceAccount
            assumeRoleARN: # <8>
              key: assume_role_arn # <8>
              secretName: cross-account-secret # <8>
  pipelines:
    - name: app-logs
      inputRefs:
        - application
...
<8> Optional: Additional role ARN to assume for cross-account access or role chaining. Specify the name of the secret and the key where the additional `role_arn` is stored.
+
...
The optional `assumeRoleARN` field enables cross-account access or additional role chaining. This is useful when you need to write logs to CloudWatch in a different AWS account or when your security setup requires assuming additional roles.

==== Use Cases
* **Cross-Account Logging**: Send logs from one AWS account to CloudWatch in another account
* **Role Chaining**: Assume an intermediate role before accessing CloudWatch resources
* **Enhanced Security**: Add an extra layer of role-based access control

==== Creating the Cross-Account Secret

. Create a secret containing the additional role ARN to assume:
+
.cross-account-secret.yaml
[source, yaml]
----
apiVersion: v1
kind: Secret
...
+
```
$ oc apply -f cross-account-secret.yaml
```

==== Important Notes

* The `assumeRoleARN` is **optional** and preserves backward compatibility
* When `assumeRoleARN` is specified, Vector uses the ServiceAccount's AWS_ROLE_ARN environment variable for primary authentication and assumes the additional role
* When `assumeRoleARN` is not specified, Vector uses the traditional credentials file and profile approach
* Both roles must have the necessary trust relationships configured in AWS IAM
* This approach follows AWS best practices for role chaining
